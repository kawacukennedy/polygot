...
temp_file="/tmp/Main_$(date +%s).java"
echo "$code" > "$temp_file"
...
# Compile
javac "$temp_file" 2> compile_error.txt
compile_exit=$?
...
# Run with timeout
class_file="${temp_file%.java}.class"
timeout 30 java -cp /tmp "$(basename "$temp_file" .java)" > output.txt 2> error.txt
run_exit=$?
...
# Clean up
rm -f "$temp_file" "$class_file" output.txt error.txt compile_error.txt