  ...
  try {
    ...
    if (q) {
      where.OR = [
        ...
        { owner: { username: { contains: q as string, mode: 'insensitive' } } }
      ];
    }
    ...
    if (author) {
      where.owner = { username: { contains: author as string, mode: 'insensitive' } };
    }
    ...
    logger.error({ error, query: req.query }, 'Search snippets error');
...
router.get('/popular-tags', async (req, res) => {
  try {
    ...
    logger.error({ error }, 'Get popular tags error');
    res.status(500).json({ message: 'Internal server error' });
  }
});
...
router.get('/users', async (req, res) => {
  try {
    ...
    logger.error({ error, query: req.query }, 'Search users error');
    res.status(500).json({ message: 'Internal server error' });
  }
});
